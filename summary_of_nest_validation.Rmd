...
tab <- loc |> 
  ...
  mutate(deployment = as.Date(deploy_dt),
         retrieval = as.Date(retrieve_dt),
         duration_days = as.numeric(retrieval - deployment)) |> 
  select(tag_id, region, deployment, retrieval, duration_days) 
```
...
We can also review the distance to the nest from each estimated point and compare to the error class rating to see how these align. In the plot below we show the distance from nest by catergory type. We can see a general trend to increase distance from the actual nest with location quality catergory, however there is a very wide variation in the distances for all catergories.
...
p5 <- ggplot(alldist, aes(Loc..quality, as.numeric(dists)))+
  geom_boxplot()+
  facet_wrap(~tag_id, scales = "free_y")+
  labs(title = "Distance from Nest by Location Quality",
       x = "Location Quality",
...
# error radius 
p4 <- ggplot(alldist, aes(Error.radius, as.numeric(dists)))+
  geom_point()+
  facet_wrap(~Loc..quality,scales = "free")+
...
Each location point has an error radius value provided in meters, we can compare the distance from actual nest to error radius to see if there is a pattern. In this graph we see the distance from nest (y) with the error radius (x). Many of these points are a greater distance than the estimated from the "error radius" values. 
...
Each location point has an error radius, we can map these to visualize the error per location quality class. Note these are based on the error radius, while the plot above shows this may be problematic so should be taken into account.
