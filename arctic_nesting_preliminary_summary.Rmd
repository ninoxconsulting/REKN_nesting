...
This package uses similar approach to asses nest locations based on biological parameters and GPS locations. The output of the NestR package is based on a number of parameters supplied by the user. These require more information to fine tune and assess the impact on the output. 
...
A cursory inspection of the breeding data provides a few key thoughts/information 

1. There appears to be sufficient information to explore the geolocation patterns of breeding birds with widespread nesting across the Arctic. 

2. Breeding records are largely from Sunbird tags and almost all are argos doppler shift estimated locations, with smallest error of <250m, This provides some challenges in narrowing down the nest locations. 

3. Initial exploration of a tag thought to be stationary shows a wide distribution of records, which spans approx 12 kms square in area. 

4. A number of methods exist to explore the data and estimate nest locations. These include density estimates (KDE, clustering ), recursive movement analysis (recurse, nestR). This provides a initial look at what is possible and needs further exploration. 

5. Some of these packages also provide methodology to estimate nest success, however these were not yet tested in this preliminary review. 
